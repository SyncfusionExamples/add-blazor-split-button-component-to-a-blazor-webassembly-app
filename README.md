# How to Add the Syncfusion Blazor Split Button to a Blazor WebAssembly App
...
# Blazor Split Button Getting Started

The [Blazor Split Button](https://www.syncfusion.com/blazor-components/blazor-split-button?utm_source=github&utm_medium=listing&utm_campaign=blazor-split-buttons-github-samples) triggers a default action when the primary button is clicked and provides a list of actions when the drop-down button is clicked. It has built-in support for icons and their positioning, varied sizes, disabling items, separators, RTL, and UI customization.
...
The Syncfusion's Blazor components library offers over 70 UI components to work with Blazor server-side and client-side (Blazor WebAssembly) projects seamlessly. In addition to Split Button, we provide popular Blazor Components such as DataGrid, Charts, Scheduler, Diagram, Word Processor, and more.
